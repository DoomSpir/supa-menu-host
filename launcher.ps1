# ═══════════════════════════════════════════════════════════════════
# v3.2 — auto-claim, local save, Zuri Menu rebrand
#   ZURI MENU  //  ANIMAL COMPANY  //  INJECTION FRAMEWORK v3.0
# ═══════════════════════════════════════════════════════════════════
...
# ── AUTO-CLAIM & LOCAL SAVE (managed by manager.py) ──────────────
$_CLAIM_TOKEN = "SETUP_REQUIRED"   # Gist-scoped token — safe to embed; can't touch repos
$_CLAIM_GIST  = "SETUP_REQUIRED"   # Gist ID for automatic claim tracking
$_DATA_PATH   = "$env:APPDATA\SupaMenu\userdata.json"

function Load-SavedData {
    if (Test-Path $_DATA_PATH) {
        try { return (Get-Content $_DATA_PATH -Raw | ConvertFrom-Json) } catch {}
    }
    return $null
}

function Save-UserData($discord, $code) {
    try {
        $dir = [System.IO.Path]::GetDirectoryName($_DATA_PATH)
        if (!(Test-Path $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }
        $obj = [ordered]@{ discord=$discord; license=$code; last_seen=(Get-Date -Format "yyyy-MM-dd") }
        [System.IO.File]::WriteAllText($_DATA_PATH,
            ($obj | ConvertTo-Json -Compress),
            [System.Text.UTF8Encoding]::new($false))
    } catch {}
}

function Push-Claim($code, $hwid, $discord) {
    if ([string]::IsNullOrEmpty($_CLAIM_TOKEN) -or $_CLAIM_TOKEN -eq "SETUP_REQUIRED") { return }
    try {
        $hdrs = @{
            Authorization = "token $_CLAIM_TOKEN"
            "User-Agent"  = "SupaMenu/3.0"
            Accept        = "application/vnd.github+json"
        }
        $content = (@{code=$code; hwid=$hwid; discord=$discord;
                      ts=(Get-Date -Format "yyyy-MM-dd HH:mm:ss")} | ConvertTo-Json -Compress)
        $body = (@{files=@{"$code.json"=@{content=$content}}} | ConvertTo-Json -Depth 5 -Compress)
        Invoke-RestMethod -Uri "https://api.github.com/gists/$_CLAIM_GIST" `
            -Method PATCH -Headers $hdrs -Body $body -ContentType "application/json" -EA SilentlyContinue | Out-Null
    } catch {}
}
...
try {
    ...
    $host.UI.RawUI.WindowTitle     = "ZURI MENU v3.0  |  Animal Company  |  VR Exploit Suite"
...
function Boot-Sequence {
    ...
    Write-Host "$P$B  ║$GR         [ INJECTION FRAMEWORK v3.0 ]  — ZURI MENU      $P$B║$R"
    Write-Host "$P$B  ╚════════════════════════════════════════════════════════╝$R"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "$P$B  ══════════════════════════════════════════════════════════$R"
    Write-Host "$LP$B  >> ZURI MENU READY. ALL SYSTEMS NOMINAL.                  $R"
    Write-Host "$P$B  ══════════════════════════════════════════════════════════$R"
...
function Send-Log($user, $ip, $hwid, $tier, $code) {
    try {
        ...
        $body = @{
            username="ZURI AUTH"
            embeds=@(@{title="⚡ Zuri Menu — $($tier.ToUpper())"; color=$col; fields=$fields; footer=@{text="Zuri v3.0"}})
...
function Do-Auth {
    ...
    if ($saved -and $saved.discord) {
        Write-Host "$P$B  ║$R  $LP  Discord username:$R"
        Write-Host "$P$B  ║$R  $GR  Saved: $C$B$($saved.discord)$R  $GR← press ENTER to keep, or type a new name$R"
        Write-Host "$P$B  ║$R"
        Write-Host -NoNewline "$P$B  ║$R  $P❯ $W"; $u = Read-Host
        if ([string]::IsNullOrWhiteSpace($u)) { $u = $saved.discord }
    } else {
    ...
    $savedLic = if ($saved -and $saved.license) { $saved.license } else { $null }
    Save-UserData $u $savedLic
    return @{ user=$u; hwid=$hwid; ip=$ip; savedCode=$savedLic }
}
...
function Do-LicensePrompt($auth) {
    # ── Auto-load saved license (skip prompt entirely on return visits) ──
    if ($auth.savedCode) {
        $code = ($auth.savedCode -replace "[-\s]","").ToUpper()
        ...
        if ($res -eq "VALID") {
            ...
            Write-Host "$P$B  ╠════════════════════════════════════════════════════════╣$R"
            Write-Host "$P$B  ║$R"
            Write-Host "$P$B  ║$R  $G  ✓ Saved license loaded — HWID confirmed.$R"
            Write-Host "$P$B  ║$R  $GR  Key: $W$($auth.savedCode)$R"
            Write-Host "$P$B  ║$R"
            Write-Host "$P$B  ╚════════════════════════════════════════════════════════╝$R"
    ...
    switch ($res) {
        "VALID" {
            ...
            Save-UserData $auth.user $raw
            Write-Host "$P$B  ╚════════════════════════════════════════════════════════╝$R"
            Start-Sleep -Milliseconds 500; return $true
        }
        "UNCLAIMED" {
            Write-Host "`r$P$B  ║$R  $YL  ⚡ First use — license accepted. Registering HWID...$R        "
            Send-Log $auth.user $auth.ip $auth.hwid "paid-firstuse" $raw
            Push-Claim $code $auth.hwid $auth.user
            Save-UserData $auth.user $raw
            Write-Host "$P$B  ║$R  $G  ✓ Done. Your key is now locked to this machine.$R"
            Write-Host "$P$B  ╚════════════════════════════════════════════════════════╝$R"
            Start-Sleep -Milliseconds 800; return $true
        }
        "HWID_MISMATCH" {
        ...
        default {
            Write-Host "`r$P$B  ║$R  $RD  ✗ Could not verify license. Check internet.$R                 "
            Write-Host "$P$B  ╚════════════════════════════════════════════════════════╝$R"
            Start-Sleep -S 2; return $false
        }
    }
